...
def resize_image_max_dim(image: Image.Image, max_dim: int = 1280) -> Image.Image:
    w, h = image.size
    if max(w, h) > max_dim:
...
async def run_inference(
    ...
    async with inference_lock:
        ...
        try:
            ...
            orig_proto_w, orig_proto_h = source_image.size

            # Downscale prototype to save memory
            source_image = resize_image_max_dim(source_image, max_dim=1280)
            proto_w, proto_h = source_image.size
            scale_w = proto_w / orig_proto_w
            scale_h = proto_h / orig_proto_h
            print(f"Prototype size after resize: {source_image.size} (scale w:{scale_w:.3f}, h:{scale_h:.3f})")
            ...
            # Extract raw bboxes and scale them to the resized prototype image
            raw_bboxes = np.array(
                [[b["x1"] * scale_w, b["y1"] * scale_h, b["x2"] * scale_w, b["y2"] * scale_h] for b in prompt_box_dicts],
                dtype=np.float64,
            )
            
            # Normalize and STRICTLY clamp coordinates
            bboxes = normalize_bboxes_to_image(raw_bboxes, proto_w, proto_h)
            bboxes[:, [0, 2]] = np.clip(bboxes[:, [0, 2]], 0, proto_w - 1)
            bboxes[:, [1, 3]] = np.clip(bboxes[:, [1, 3]], 0, proto_h - 1)
            ...
            # Downscale target image to prevent massive post-processing OOM
            target_image_pil = resize_image_max_dim(target_image_pil, max_dim=1280)
            print(f"Target size after resize: {target_image_pil.size}")
            ...
            try:
                ...
                print("  [Memory Management] Cleaning up VP model...")
                if 'vp_model' in globals():
                    vp_model = None
                gc.collect()
                if DEVICE == "cuda":
